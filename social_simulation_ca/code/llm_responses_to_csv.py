...
def extract_responses_to_csv(input_json_path: str, output_csv_path: str):
    ...
    """
    # 读取LLM用户回答数据
    print(f"Reading LLM user responses from: {input_json_path}")
    ...
    for profile_id, user_info in user_responses.items():
        ...
        print(f"Processed user: {profile_id} (Country: {country_name})")

    
    # 按profile_id排序
    user_data_list.sort(key=lambda x: int(x['profile_id']))
...
def main():
    # 获取文件路径
    input_json_path = "/home/zhou/persona/world_value_survey/gpt-4.1-mini_ca/output/llm_responses.json"
    output_csv_path = "/home/zhou/persona/world_value_survey/gpt-4.1-mini_ca/output/llm_responses.csv"
