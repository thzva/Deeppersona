        ...
        try:
            ...
            if "paths" in self.attributes:
                ...
                for category in all_recommended:
                    category_paths = self._flatten_attributes(self._get_nested_attributes(category), category)
                    all_paths.extend(category_paths)
            
            # 使用指定的属性数量
            final_count = min(target_count, len(all_paths))
            final_paths = random.sample(all_paths, final_count)
            logger.info(f"从所有 {len(all_paths)} 条属性中选择了 {final_count} 条属性作为最终返回值")
...
def get_selected_attributes(user_profile=None, attribute_count=200):
    ...
    if ATTRIBUTE_SELECTION_CACHE is not None and len(ATTRIBUTE_SELECTION_CACHE) == attribute_count:
    ...
    try:
        ...
        # 获取属性列表，传入指定的属性数量
        top_paths = selector.get_top_attributes(attribute_recommendations, attribute_count)
